...
class SwatEnv(gym.Env):
    ...
    def clear_flows(self):
        for switch in self.env.net.switches:
            switch.cmd('ovs-ofctl del-flows', switch)
            switch.cmd('ovs-ofctl add-flow', switch, 'table=0,priority=0,actions=CONTROLLER:5555')

    def reset_interfaces(self):
        for host in self.env.net.hosts:
            for intf in host.intfList():
                host.cmd(f'ifconfig {intf.name} down')
                host.cmd(f'ifconfig {intf.name} up')
